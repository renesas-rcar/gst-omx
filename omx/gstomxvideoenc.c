...
gst_omx_video_enc_init (GstOMXVideoEnc * self)
{
  ...
  self->extaddr_array = g_array_new (FALSE, FALSE, sizeof (OMXR_MC_VIDEO_EXTEND_ADDRESSTYPE));
#endif
    ...
    GstBuffer * input)
{
  ...
  return gst_omx_video_enc_update_input_port (self, port_def, stride, slice_height);
}
#endif
...
update_input_port_color_format (GstOMXVideoEnc * self)
{
  ...
  if (gst_omx_port_update_port_definition (self->enc_in_port,&port_def) != OMX_ErrorNone)
    return FALSE;
...
static gboolean
mmngr_import_dmabuf (GstOMXVideoEnc * self, gint fd, gint *id, guint *phys_addr)
{
  gsize size;
  gint ret;

  ret =
      mmngr_import_start_in_user_ext (id, &size, phys_addr, fd,
      NULL);
  if (ret != R_MM_OK) {
    ...
    GstVideoCodecState * state)
{
  ...
  if (!gst_omx_video_enc_update_input_port (self, port_def, info->width, info->height))
    return FALSE;
    ...
    GstVideoCodecFrame * frame)
{
  ...
  if (self->import_dmabuf) {
    ...
    if (!table && self->fd_table_array->len == port->port_def.nBufferCountActual) {
          GST_ERROR_OBJECT (self,
