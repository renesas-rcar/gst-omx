    ...
    GstVideoCodecState * state)
{
  ...
  if (strncmp (fourcc, "WVC1", strlen ("WVC1")) == 0) {
    GST_DEBUG_OBJECT (dec, "Handling for VC-1 stream - Advanced profile");
  ...
  if (state->codec_data) {
    ...
    if (!self->advanced_profile) {
       ...
       */
      if (!gst_buffer_map (state->codec_data, &info, GST_MAP_READ)) {
      ...
      gst_buffer_unmap (state->codec_data, &info);

      codec_data = gst_buffer_new_wrapped (SeqHdrBuf, SEQ_PARAM_BUF_SIZE);

      if (!gst_buffer_replace (&state->codec_data, codec_data)) {
        GST_ERROR_OBJECT (dec, "Failed to replace analysed codec_data");
        return GST_FLOW_ERROR;
      }
